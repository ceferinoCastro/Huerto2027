import {ApiError,actualizarCampania,actualizarAsociacion,actualizarCartel,analizarCsvHanna,cancelarCampania,corregirAsignacionHanna,crearAsociacion,crearCampania,crearCartel,eliminarAsociacion,eliminarCartel,fetchAsociaciones,fetchCampanias,fetchCampaniaActiva,fetchCarteles,fetchColegios,fetchDataloggersHanna,fetchMedicionesPlantas,fetchPromediosPlantas,fetchResumenLocalidad,fetchSensores,fetchUltimasEducativas,finalizarCampania,guardarMedicionesPlantas,importarCsvHanna,reemplazarHanna} from "./api.js?v=6";
...
const state={localidad:"pica",colegios:[],sensoresCatalogo:[],resumen:null,sensores:[],variables:[],asociaciones:[],apiAsociacionesDisponible:null,cargandoAsociaciones:false,asociacionesVacias:false,errorConsultaAsociaciones:null,errorOperacionAsociacion:null,editando:null,carga:0,cargandoConfiguracion:true};
...
let campanaActivaAsociaciones=null;
async function actualizarCampanaActivaAsociaciones(){
 const colegio=colegioActual();
 if(!colegio){campanaActivaAsociaciones=null;return}
 try{const data=await fetchCampaniaActiva(colegio._id||colegio.colegio_id||colegio.id);campanaActivaAsociaciones=data.item?.estado==="activa"?data.item:null}
 catch{campanaActivaAsociaciones=null}
}
const cartelState={items:[],cargando:false,error:null,editando:null,sugeridos:{fuente_sugerida:null,variable_tecnica_sugerida:null,sensor_modelo_sugerido:null},regla:null};
...
const etiquetaSemaforo={green:"Dentro del rango óptimo",yellow:"Cerca del límite del rango",red:"Fuera del rango configurado"};
function puntoEstadoSemaforo(item){
 const campania=campanaActivaAsociaciones&&campanaActivaAsociaciones.colegio_id===item.colegio_id?campanaActivaAsociaciones:null;
 if(!campania)return "";
 const estado=evaluarSemaforo(item.valor,campania.rangos_variables?.[item.variable]);
 if(!estado)return "";
 return '<span class="status-dot status-dot-'+estado+'" aria-hidden="true" title="'+escapar(etiquetaSemaforo[estado])+'"></span>';
}
...
async function cargarVisorCarteles(forzar=false){
 ...
 finally{if(carga===posterState.carga&&localidad===state.localidad){posterState.cargando=false;$("#test-associations").disabled=false;renderVisorCarteles();renderAsociaciones();opcionesCarteles()}}
}
function opcionesCarteles(){
 const select=$("#clave-educativa"),seleccion=select.value;
 select.innerHTML='<option value="">Selecciona un cartel</option>'+posterState.catalogo.map(item=>'<option value="'+escapar(item.clave_educativa)+'">'+escapar(item.nombre_educativo)+' ('+escapar(item.clave_educativa)+')</option>').join("");
...
function prepararAsociacion(key){const definition=posterState.catalogo.find(item=>item.clave_educativa===key);if(!definition)return;resetFormulario();$("#clave-educativa").value=definition.clave_educativa;$("#association-form").scrollIntoView({behavior:"smooth",block:"start"})}
...
function resetFormulario(){state.editando=null;$("#association-form").reset();$("#association-id").value="";$("#form-title").textContent="Crear asociación";$("#cancel-edit").hidden=true;opcionesFormulario();opcionesCarteles()}
function editar(id){const item=state.asociaciones.find(a=>(a._id||a.id)===id);if(!item)return;state.editando=id;Object.entries({"clave-educativa":item.clave_educativa,"sensor-sn":item.sensor_sn,"variable-tecnica":item.variable_tecnica,"ubicacion":item.ubicacion,"profundidad-cm":item.profundidad_cm}).forEach(([idCampo,valor])=>{$("#"+idCampo).value=valor??""});$("#form-title").textContent="Editar asociación";$("#cancel-edit").hidden=false;actualizarVariablesFormulario();window.scrollTo({top:$("#association-form").offsetTop-100,behavior:"smooth"})}
function payloadFormulario(){const colegio=colegioActual();return {localidad:state.localidad,colegio_id:colegio?._id||null,device_sn:state.resumen?.device_sn||null,clave_educativa:$("#clave-educativa").value.trim(),sensor_sn:$("#sensor-sn").value,variable_tecnica:$("#variable-tecnica").value,ubicacion:$("#ubicacion").value.trim()||null,profundidad_cm:$("#profundidad-cm").value===""?null:Number($("#profundidad-cm").value)}}
...
function renderCarteles(){
 if(cartelState.cargando){$("#cartel-count").textContent="Cargando…";$("#carteles-body").innerHTML='<tr><td colspan="6" class="empty-cell">Cargando carteles…</td></tr>';return}
 if(cartelState.error){$("#cartel-count").textContent="Error";$("#carteles-body").innerHTML='<tr><td colspan="6" class="empty-cell">'+escapar(cartelState.error)+'</td></tr>';return}
 $("#cartel-count").textContent=cartelState.items.length+" carteles";
 $("#carteles-body").innerHTML=cartelState.items.length?cartelState.items.map(item=>'<tr><td>'+escapar(item.orden)+'</td><td><strong>'+escapar(item.nombre_educativo)+'</strong><small>'+escapar(item.clave_educativa)+'</small></td><td>'+escapar(item.categoria||"—")+'</td><td>'+escapar(item.unidad||"—")+'</td><td>'+(item.visible_frontend===false?"No":"Sí")+'</td><td><div class="row-actions"><button type="button" data-edit-cartel="'+escapar(item.clave_educativa)+'">Editar</button><button type="button" class="delete" data-delete-cartel="'+escapar(item.clave_educativa)+'">Eliminar</button></div></td></tr>').join(""):'<tr><td colspan="6" class="empty-cell">Sin carteles registrados.</td></tr>';
 document.querySelectorAll("[data-edit-cartel]").forEach(button=>button.onclick=()=>editarCartel(button.dataset.editCartel));
 document.querySelectorAll("[data-delete-cartel]").forEach(button=>button.onclick=()=>borrarCartel(button.dataset.deleteCartel));
}
async function cargarCarteles(){
 cartelState.cargando=true;cartelState.error=null;renderCarteles();
 try{const data=await fetchCarteles();cartelState.items=data.items||[]}
 catch(error){cartelState.error=error.message}
 finally{cartelState.cargando=false;renderCarteles()}
}
const camposVisualesCartel={icono:"#cartel-icono",grupo_visual:"#cartel-grupo-visual",compartir_conexion_con:"#cartel-compartir-conexion",posicion_x:"#cartel-posicion-x",posicion_y:"#cartel-posicion-y",destino_x:"#cartel-destino-x",destino_y:"#cartel-destino-y",destino_zona:"#cartel-destino-zona",ruta_punto1_x:"#cartel-ruta1-x",ruta_punto1_y:"#cartel-ruta1-y",ruta_punto2_x:"#cartel-ruta2-x",ruta_punto2_y:"#cartel-ruta2-y",ancla:"#cartel-ancla",etiqueta_corta:"#cartel-etiqueta-corta",explicacion:"#cartel-explicacion"};
const camposVisualesNumericos=new Set(["posicion_x","posicion_y","destino_x","destino_y","ruta_punto1_x","ruta_punto1_y","ruta_punto2_x","ruta_punto2_y"]);
function payloadVisualCartel(){
 const payload={};
 for(const [campo,selector] of Object.entries(camposVisualesCartel)){
  const valor=$(selector).value.trim();
  payload[campo]=valor===""?null:camposVisualesNumericos.has(campo)?Number(valor):valor;
 }
 payload.mostrar_conexion=$("#cartel-mostrar-conexion").checked;
 return payload;
}
function payloadFormularioCartel(){return {clave_educativa:$("#cartel-clave").value.trim(),nombre_educativo:$("#cartel-nombre").value.trim(),unidad:$("#cartel-unidad").value.trim(),categoria:$("#cartel-categoria").value,orden:Number($("#cartel-orden").value),visible_frontend:$("#cartel-visible").checked,...cartelState.sugeridos,...payloadVisualCartel(),regla:cartelState.regla}}
function resetFormularioCartel(){cartelState.editando=null;cartelState.sugeridos={fuente_sugerida:null,variable_tecnica_sugerida:null,sensor_modelo_sugerido:null};cartelState.regla=null;$("#cartel-form").reset();$("#cartel-clave").disabled=false;$("#cartel-orden").value="1";$("#cartel-visible").checked=true;$("#cartel-mostrar-conexion").checked=true;for(const selector of Object.values(camposVisualesCartel))$(selector).value="";$("#cartel-form-title").textContent="Crear cartel";$("#cartel-cancel-edit").hidden=true}
function editarCartel(clave){const item=cartelState.items.find(c=>c.clave_educativa===clave);if(!item)return;cartelState.editando=clave;cartelState.sugeridos={fuente_sugerida:item.fuente_sugerida??null,variable_tecnica_sugerida:item.variable_tecnica_sugerida??null,sensor_modelo_sugerido:item.sensor_modelo_sugerido??null};cartelState.regla=item.regla??null;$("#cartel-clave").value=item.clave_educativa;$("#cartel-clave").disabled=true;$("#cartel-nombre").value=item.nombre_educativo;$("#cartel-unidad").value=item.unidad||"";$("#cartel-categoria").value=item.categoria||"otra";$("#cartel-orden").value=item.orden;$("#cartel-visible").checked=Boolean(item.visible_frontend);for(const [campo,selector] of Object.entries(camposVisualesCartel))$(selector).value=item[campo]??"";$("#cartel-mostrar-conexion").checked=item.mostrar_conexion!==false;$("#cartel-form-title").textContent="Editar cartel";$("#cartel-cancel-edit").hidden=false;window.scrollTo({top:$("#cartel-form").offsetTop-100,behavior:"smooth"})}
async function guardarCartel(event){
 event.preventDefault();setStatus("loading","Guardando cartel…");
 try{const payload=payloadFormularioCartel();if(cartelState.editando)await actualizarCartel(cartelState.editando,payload);else await crearCartel(payload);resetFormularioCartel();await cargarCarteles();await cargarVisorCarteles(true);setStatus("success","Cartel guardado exitosamente.")}
 catch(error){setStatus("error","No fue posible guardar el cartel. "+error.message)}
}
async function borrarCartel(clave){
 if(!confirm("¿Eliminar este cartel? Fallará si tiene asociaciones activas."))return;
 setStatus("loading","Eliminando cartel…");
 try{await eliminarCartel(clave);await cargarCarteles();await cargarVisorCarteles(true);setStatus("success","Cartel eliminado.")}
 catch(error){setStatus("error","No fue posible eliminar el cartel. "+error.message)}
}
async function cargarLocalidad(resultado){
 ...
 try{
  const [colegios,sensores,resumen]=await Promise.all([fetchColegios(),fetchSensores(),fetchResumenLocalidad(state.localidad)]);if(carga!==state.carga)return;
  state.colegios=colegios.items||[];state.sensoresCatalogo=sensores.items||[];state.resumen=resumen;state.sensores=agruparResumen(resumen.items||[]);state.variables=[...new Map((resumen.items||[]).filter(i=>i.variable).map(i=>[i.variable,{nombre:i.variable,unidad:i.units||""}])).values()];
  actualizarCampanaActivaAsociaciones().then(()=>{if(carga===state.carga)renderVisorCarteles()});
...
function calcularPromedioDia(){
 ...
 const raizNumero=Number($("#plants-root-length").value),raizValida=Number.isFinite(raizNumero)&&raizNumero>0;
 $("#daily-root-length").textContent=raizValida?raizNumero.toLocaleString("es-CL",{minimumFractionDigits:1,maximumFractionDigits:2}):"Sin dato";
}
function limpiarFormularioPlantas(){document.querySelectorAll(".plant-height").forEach(input=>input.value="");$("#plants-root-length").value="";$("#plants-observation").value="";calcularPromedioDia()}
function rellenarFormularioPlantas(items){
 limpiarFormularioPlantas();items.forEach(item=>{const input=document.querySelector('.plant-height[data-plant="'+item.planta_numero+'"]');if(input&&item.altura_cm!=null)input.value=item.altura_cm});const raizGuardada=items.find(item=>item.largo_raiz_cm!=null)?.largo_raiz_cm;if(raizGuardada!=null)$("#plants-root-length").value=raizGuardada;$("#plants-observation").value=items.find(item=>item.observacion)?.observacion||"";calcularPromedioDia();
}
...
function valorNumericoONulo(valor){return Number.isFinite(Number(valor))&&Number(valor)>0?Number(valor):null}
function tramosSerie(items,campo){
 const tramos=[];let actual=[];
 items.forEach((item,i)=>{const valor=valorNumericoONulo(item[campo]);if(valor===null){if(actual.length)tramos.push(actual);actual=[];return}actual.push({i,valor,fecha:item.fecha})});
 if(actual.length)tramos.push(actual);
 return tramos;
}
function ultimoConDato(items,campo){return [...items].reverse().find(item=>valorNumericoONulo(item[campo])!==null)||null}
function renderGraficoPlantas(){
 ...
 const ultimoAltura=ultimoConDato(items,"altura_promedio_cm"),ultimoRaiz=ultimoConDato(items,"largo_raiz_cm");
 $("#last-average").textContent=ultimoAltura?Number(ultimoAltura.altura_promedio_cm).toLocaleString("es-CL",{maximumFractionDigits:2}):"—";
 $("#last-average-date").textContent=ultimoAltura?"Fecha "+ultimoAltura.fecha+" · "+ultimoAltura.plantas_medidas+" plantas medidas":"Sin datos";
 $("#last-root-length").textContent=ultimoRaiz?Number(ultimoRaiz.largo_raiz_cm).toLocaleString("es-CL",{maximumFractionDigits:2}):"—";
 $("#last-root-length-date").textContent=ultimoRaiz?"Fecha "+ultimoRaiz.fecha:"Sin datos";
 if(!items.length){$("#plant-chart").innerHTML='<div class="chart-empty"><strong>Aún no hay mediciones de plantas para este huerto</strong><span>Guarda alturas o el largo de la raíz para comenzar el historial real.</span></div>';return}
 const width=560,height=280,left=48,right=18,top=22,bottom=44;
 const valoresAltura=items.map(item=>valorNumericoONulo(item.altura_promedio_cm)).filter(Number.isFinite);
 const valoresRaiz=items.map(item=>valorNumericoONulo(item.largo_raiz_cm)).filter(Number.isFinite);
 const max=Math.max(1,...valoresAltura,...valoresRaiz),yMax=Math.max(5,Math.ceil(max/5)*5);
 const x=(i)=>items.length===1?(left+width-right)/2:left+i*((width-left-right)/(items.length-1)),y=value=>top+(yMax-value)*((height-top-bottom)/yMax);
 let svg='<svg viewBox="0 0 '+width+' '+height+'" role="img" aria-label="Altura de planta y largo de raíz por día, fechas en el eje horizontal y centímetros en el eje vertical"><text class="axis-label" x="14" y="'+(height/2)+'" text-anchor="middle" transform="rotate(-90 14 '+(height/2)+')">Centímetros</text>';
 ...
 const dibujarSerie=(campo,claseExtra,etiqueta)=>{
  tramosSerie(items,campo).forEach(tramo=>{
   const puntos=tramo.map(punto=>x(punto.i)+","+y(punto.valor)).join(" ");
   svg+='<polyline class="line '+claseExtra+'" points="'+puntos+'"/>';
   tramo.forEach(punto=>svg+='<circle class="point '+claseExtra+'" cx="'+x(punto.i)+'" cy="'+y(punto.valor)+'" r="5"><title>'+escapar(etiqueta)+' · '+escapar(punto.fecha)+': '+escapar(punto.valor)+' cm</title></circle>')
  });
 };
 dibujarSerie("altura_promedio_cm","","Altura de planta");
 dibujarSerie("largo_raiz_cm","root","Largo de raíz");
...
async function cargarPromediosPlantas(mostrarEstado=true){
 ...
 try{if(!state.colegios.length){const colegios=await fetchColegios();state.colegios=colegios.items||[];renderContextoPlantas()}const data=await fetchPromediosPlantas(plantState.localidad,30);if(carga!==plantState.carga)return;
  plantState.promedios=(data.items||[]).filter(item=>{
   const alturaValida=Number.isFinite(Number(item.altura_promedio_cm))&&Number(item.altura_promedio_cm)>0&&Number(item.plantas_medidas)>0;
   const raizValida=Number.isFinite(Number(item.largo_raiz_cm))&&Number(item.largo_raiz_cm)>0;
   return alturaValida||raizValida;
  }).sort((a,b)=>a.fecha.localeCompare(b.fecha));
  renderGraficoPlantas();if(mostrarEstado)setStatus(plantState.promedios.length?"success":"empty",plantState.promedios.length?"Promedios reales de los últimos 30 días cargados.":"Aún no hay mediciones de plantas para este huerto.")}
...
async function guardarPlantas(event){
 event.preventDefault();const mediciones=medicionesFormulario(),valorRaiz=$("#plants-root-length").value.trim(),largoRaizCm=valorRaiz===""?null:Number(valorRaiz);
 if(!mediciones.length&&largoRaizCm===null){setStatus("empty","Ingresa al menos una altura o el largo de la raíz antes de guardar.");return}
 ...
 if(largoRaizCm!==null&&(!Number.isFinite(largoRaizCm)||largoRaizCm<=0)){setStatus("error","El largo de la raíz debe ser un número positivo.");return}
 const boton=$("#save-plants"),colegio=colegioPorLocalidad(plantState.localidad),payload={localidad:plantState.localidad,colegio_id:colegio?._id||null,huerto_id:null,ciclo_id:"general",fecha:$("#plants-fecha").value,mediciones,largo_raiz_cm:largoRaizCm,observacion:$("#plants-observation").value.trim()||null};
 boton.disabled=true;boton.textContent="Guardando…";setStatus("loading","Guardando "+(mediciones.length||"la")+" "+(mediciones.length===1?"medición":"mediciones")+"…");
 try{
  const result=await guardarMedicionesPlantas(payload),confirmacion=await cargarMedicionesFecha(false);await cargarPromediosPlantas(false);
  const alturasConfirmadas=confirmacion&&mediciones.every(enviada=>{const guardada=confirmacion.items.find(item=>item.planta_numero===enviada.planta_numero);return guardada&&Number(guardada.altura_cm)===enviada.altura_cm});
  const raizConfirmada=largoRaizCm===null||(confirmacion&&confirmacion.items.some(item=>Number(item.largo_raiz_cm)===largoRaizCm));
  if(!alturasConfirmadas||!raizConfirmada)throw new Error("el servidor no devolvió todas las mediciones guardadas");
  setStatus("success","Mediciones guardadas y verificadas: "+result.created+" nuevas y "+result.updated+" actualizadas.")
 }
...
let cargandoCatalogoParaResumen=false;
function renderCampanias(){
 const active=campaignState.activa?.estado==="activa"?campaignState.activa:null,activeBox=$("#campaign-active"),newPanel=document.querySelector(".campaign-new-panel"),overviewPanel=document.querySelector(".campaign-overview-panel"),newButton=$("#campaign-new-trigger"),help=$("#campaign-new-help"),blocked=Boolean(active);
 newButton.disabled=!campaignState.colegioId||blocked;help.hidden=!blocked;
 const mostrarFormulario=campaignState.formOpen,mostrarResumen=Boolean(active)&&!mostrarFormulario;
 newPanel.hidden=!mostrarFormulario;overviewPanel.hidden=!mostrarResumen;
 document.querySelector(".campaign-layout").classList.toggle("form-closed",!mostrarFormulario&&!mostrarResumen);
 if(active){
  if(!posterState.catalogo.length&&!cargandoCatalogoParaResumen){cargandoCatalogoParaResumen=true;asegurarCatalogoCarteles().then(()=>{cargandoCatalogoParaResumen=false;renderCampanias()})}
  const elapsed=Math.max(0,Math.floor((Date.now()-Date.parse(active.fecha_siembra+"T00:00:00"))/86400000));
  activeBox.innerHTML='<div class="campaign-card"><h3>'+escapar(active.nombre)+'</h3><div class="campaign-details"><div><small>Cultivo</small><strong>'+escapar(active.cultivo)+'</strong></div><div><small>Estado</small><strong>'+escapar(active.estado)+'</strong></div><div><small>Siembra</small><strong>'+escapar(active.fecha_siembra)+'</strong></div><div><small>Días transcurridos</small><strong>'+elapsed+'</strong></div><div><small>Cosecha estimada</small><strong>'+escapar(active.fecha_cosecha_estimada)+'</strong></div><div><small>Observaciones</small><strong>'+escapar(active.observaciones||"Sin observaciones")+'</strong></div></div><div class="campaign-actions"><button class="primary-button" type="button" id="campaign-edit">Editar campaña</button><button class="primary-button" type="button" id="campaign-finish">Finalizar campaña</button><button class="primary-button danger" type="button" id="campaign-cancel">Cancelar campaña</button></div></div>';
  $("#campaign-edit").onclick=editarCampaniaActiva;$("#campaign-finish").onclick=finalizarCampaniaActiva;$("#campaign-cancel").onclick=cancelarCampaniaActiva;
  if(mostrarResumen)renderResumenUmbrales(active.rangos_variables||{},"campaign-overview-body");
 }else activeBox.innerHTML='<p class="empty">Este colegio no tiene una campaña activa. Usa “+ Nueva campaña” para comenzar.</p>';
 const history=campaignState.items.filter(item=>item.estado!=="activa");$("#campaign-count").textContent=campaignState.items.length+" "+(campaignState.items.length===1?"campaña":"campañas");$("#campaign-history").innerHTML=history.length?'<div class="campaign-history-list">'+history.map(item=>'<details class="campaign-history-item"><summary><strong>'+escapar(item.nombre)+'</strong><span>'+escapar(item.cultivo)+'</span></summary><div><small>Siembra</small><strong>'+escapar(item.fecha_siembra||"—")+'</strong></div><div><small>Cosecha real</small><strong>'+escapar(item.fecha_cosecha_real||"—")+'</strong></div><div><small>Estado</small><strong>'+escapar(item.estado)+'</strong></div></details>').join("")+'</div>':'<p class="empty">Sin campañas anteriores.</p>'
}
...
async function asegurarCatalogoCarteles(){if(posterState.catalogo.length)return;await cargarVisorCarteles(true)}
function renderRangosCampania(rangos={}){
 const cuerpo=$("#campaign-ranges-body");
 if(!posterState.catalogo.length){cuerpo.innerHTML='<p class="empty">No fue posible cargar el catálogo de carteles.</p>';return}
 cuerpo.innerHTML=posterState.catalogo.map(item=>{
  const rango=rangos[item.clave_educativa]||{};
  return '<div class="campaign-ranges-row" data-range-key="'+escapar(item.clave_educativa)+'">'
   +'<span class="campaign-ranges-label">'+escapar(item.nombre_educativo)+' <small>'+escapar(item.clave_educativa)+'</small> <em>('+escapar(item.unidad||"—")+')</em></span>'
   +'<input type="number" step="any" data-range-field="min" value="'+escapar(rango.min??"")+'" placeholder="Mín." aria-label="Mínimo de '+escapar(item.nombre_educativo)+'">'
   +'<input type="number" step="any" data-range-field="max" value="'+escapar(rango.max??"")+'" placeholder="Máx." aria-label="Máximo de '+escapar(item.nombre_educativo)+'">'
   +'</div>';
 }).join("");
}
function payloadRangosCampania(){
 const rangos={};
 document.querySelectorAll("#campaign-ranges-body [data-range-key]").forEach(fila=>{
  const clave=fila.dataset.rangeKey;
  const min=fila.querySelector('[data-range-field="min"]').value.trim();
  const max=fila.querySelector('[data-range-field="max"]').value.trim();
  if(min===""&&max==="")return;
  rangos[clave]={min:min===""?null:Number(min),max:max===""?null:Number(max)};
 });
 return rangos;
}
function renderResumenUmbrales(rangos,contenedorId){
 const cuerpo=$("#"+contenedorId);
 if(!posterState.catalogo.length){cuerpo.innerHTML='<p class="empty">Cargando catálogo de carteles…</p>';return}
 cuerpo.innerHTML=posterState.catalogo.map(item=>{
  const rango=rangos[item.clave_educativa]||{},min=rango.min,max=rango.max,definido=min!=null||max!=null;
  const texto=definido?"Mín: "+(min??"—")+" | Máx: "+(max??"—"):"Sin umbrales definidos";
  return '<div class="campaign-range-summary-item"><span>'+escapar(item.nombre_educativo)+'</span><em>'+escapar(texto)+'</em></div>';
 }).join("");
}
function abrirFormularioCampania(mode){
 ...
 $("#campaign-ranges-body").innerHTML='<p class="empty">Cargando catálogo de carteles…</p>';
 asegurarCatalogoCarteles().then(()=>renderRangosCampania(item?.rangos_variables||{}));
...
async function iniciarCampania(event){
 ...
 const rangos=payloadRangosCampania(),rangosAntes=JSON.stringify(editing?.rangos_variables||{});
 if(!editing||JSON.stringify(rangos)!==rangosAntes)payload.rangos_variables=rangos;
 ...
 try{
  ...
  actualizarCampanaActivaAsociaciones().then(renderVisorCarteles);
...
function seleccionarTab(nombre){
 ...
 if(nombre==="plants"){plantState.localidad=state.localidad;$("#plants-localidad").value=plantState.localidad;renderContextoPlantas();cargarVistaPlantas()}else if(nombre==="hanna")cargarEquiposHanna();else if(nombre==="campaigns")prepararSelectorCampanias().then(cargarCampanias);else if(nombre==="posters")cargarCarteles();else setStatus(state.sensores.length?"success":"empty",state.sensores.length?"Información técnica cargada correctamente.":"La localidad no tiene datos de sensores disponibles.");
}
...
$("#cartel-form").onsubmit=guardarCartel;
$("#cartel-cancel-edit").onclick=resetFormularioCartel;
...
$("#plants-root-length").addEventListener("input",calcularPromedioDia);
